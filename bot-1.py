...
from video import create_simple_video, OUTPUT_DIR
from uploader import get_public_video_url
...
async def post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        public_url = get_public_video_url(path)
...
class _HealthHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        # /video/<filename> so'ralsa - shu video faylni qaytaradi (Instagram shundan o'qiydi)
        if self.path.startswith("/video/"):
            filename = self.path.replace("/video/", "").split("?")[0]
